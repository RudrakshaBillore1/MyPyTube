...
#Fuction to get path
def select_path():
    #allowauser to select a path from the explorer
    ...
    try:
        ...
        # checking if the comboxbox value is None
            ...
            try:
                ...
                # downlaoding the actual video  edited
                video.streams.filter(res=resolution).first().download(user_path)
...
url_entry = ttk.Entry(window, width=56, style='TEntry')
...
# adding the label to the canvas
canvas.create_window(60, 260, window=resolution_label)
...
canvas.create_window(70, 280, window=video_resolution)
...
canvas.create_window(95, 315, window=search_resolution)

#Select path for savingthe file
path_label = Label(window, text="Select Path For Download", font=('Arial',14))
select_btn = Button(window , text="Select Path" , bg='red', padx='22', pady='5',  font=('Arial', 14), fg='#fff', command=select_path)
canvas.create_window(250,315, window=select_btn)
